...
- Provider-neutral visual-treatment foundation between `ProductionTimeline` and `RenderProvider`, including deterministic still-image motion assignment, conservative bounded crossfade transitions, native-motion preservation for generated video, and FFmpeg-backed local realization without coupling the contract to any single premium video provider
- Provider-neutral caption-styling foundation between `ProductionTimeline` and `RenderProvider`, including deterministic emphasis modes, readable wrapping, safe-area policy, FFmpeg-backed ASS realization, and unchanged caption timing across still-image, generated-video, and mixed-media timelines
- Provider-neutral audio-layer foundation between `ShortRenderRequest` and `RenderProvider`, including optional background music, optional sound effects, deterministic fades and gain control, narration-preserving ducking intent, offline FFmpeg final mixing, and unchanged timeline/caption/visual-treatment behavior when audio layers are absent
...
- Timeline-aware transitions, word-level caption timing, AI music or SFX generation providers, richer sound design, and further render-quality refinements only when later milestones intentionally start that work
